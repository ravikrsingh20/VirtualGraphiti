...
function moveforMap(frame) {
	...
	if (frame.valid && frame.gestures.length > 0) {
		frame.gestures.forEach(function(gesture) {
			...
			filterGesture("swipe", swipe)(frame, gesture);
		});
	}
...
function swipe(frame, keyTapGesture){
	var yes = confirm("Do you want to save Location for the Graphiti.");
	if (yes == true) {
		var coordinate = document.getElementById("coord").innerHTML;
		var dataToPost = {coord:coordinate};
		$.ajax({
		  type: "POST",
		  url: "http://localhost:8080/vg/upload/location",
		  data: dataToPost,
		  success: function(data) {
			  alert('Location saved for Graphiti ');
			  window.location.href = "http://localhost:8080/vg/viewGraphiti";
		  },error: function (jqXHR, textStatus, errorThrown) {
			  if(jqXHR.status==400){
				  alert("Please select other location. Some Graphiti exits within 20m radius of location "+coordinate);
			  }
		  }
		});
	} else {
